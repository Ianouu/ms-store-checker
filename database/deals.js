...
const db = connection.database('deals');
...
*/
 ...
 function createDBifNotExist() {
    ...
    MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
        db_mongo.collection('deals', {}, function(err, deals) {
            deals.remove({}, function(err, result) {
 ...
 function addOne( obj) {
    if ( obj.id_microsoft && obj.id_microsoft != '') {
        ...
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            ...
            db_mongo.collection('deals').insertOne( obj,
              function (err, res) {
                if (err) {
                    db_mongo.close();
                  return console.log(err);
...
function getAll() {
    ...
    return new Promise((resolve, result) => {
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            db_mongo.collection('deals').find({}).toArray(function(err, result) {
                if ( err) reject(err);
                else {
                    resolve(result);
                    db_mongo.close();
 ...
 async function updateALLUSDPrice() {
    ...
    for( let i=0; i< all.length; i++) {
        ...
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            ...
            db_mongo.collection("deals").updateOne({"id_microsoft" : tmp.id}, newvalues, function(err, res) {
                if (err) throw err;
                db_mongo.close();
            });
        });
