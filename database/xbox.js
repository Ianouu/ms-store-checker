...
const db = connection.database('xbox');
 ...
 function createDBifNotExist() {
    ...
    // } else {
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            db_mongo.collection('xbox', {}, function(err, xbox) {
 ...
 function addOne( obj) {
    if ( obj.id_microsoft && obj.id_microsoft != '') {
        ...
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            ...
            db_mongo.collection('xbox').insertOne( obj,
              function (err, res) {
                if (err) {
                    db_mongo.close();
                  return console.log(err);
...
function getAll() {
    return new Promise((resolve, result) => {
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            db_mongo.collection('xbox').find({}).toArray(function(err, result) {
                if ( err) reject(err);
                else {
                    resolve(result);
                    db_mongo.close();
    ...
    // // all = await p;
    // p.then((result) => {console.log(result.length);return result});
}
 ...
 async function getCheapestByMicrosoftID( id) {
    ...
    if ( min_uuid !== -1) {
        return(db.getSync(min_uuid)); // TODO : supprimer le db.getsync -> singnifier utilise le nosqlite
    } else {
        return(null);
    }
}
 ...
 async function updateALLUSDPrice() {
    ...
    for( let i=0; i< all.length; i++) {
        ...
        MongoClient.connect(MONGO_URL, (err, db_mongo) => {  
            ...
            db_mongo.collection("xbox").updateOne({"id_microsoft" : tmp.id}, newvalues, function(err, res) {
                if (err) throw err;
                db_mongo.close();
              });
          });
